...
import xlo.bitmap.FixedArrayBitMap;
 ...
 * @title
 *
 * TODO 重写了BitMap因此需要修改连接池
 * TODO 重写了BitMap因此需要修改连接池
 * TODO 连接Connection时添加自动关闭时间：当服务一定时间没有响应时，数据库端关闭该连接(防止服务关闭而数据库自动关闭时间过长，造成堵塞)
 * TODO 连接池添加定时未使用自动回收
 * TODO 连接池添加在运行时设置参数
 * TODO 添加JVM关闭钩子(程序正常结束时释放Connection资源)
 *
 */
...
public class ConnPool {
	 ...
	 */
	private FixedArrayBitMap connStates= null;
	/**
	 * 连接池管理线程
	 */
	...
	public void start(){
		...
		// todo 原本BITMAP是不定长的，因为重写了bitMap，将其改为定长，
		this.connStates = new FixedArrayBitMap(255);
		this.connStates.reverse();
...
//		connStates.setSize(this.pool.size());
		...
		throw new NoSuchMethodError("Unsupported method.");
	}
